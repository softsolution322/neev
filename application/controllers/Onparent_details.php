...
class Onparent_details extends MY_controller {
	...
	public function get_pay_details()
	{
		...
		foreach($month as $key => $month_key){
			...
			$tot += $data[0]->TOTAL;
			$total_amount = $tot + $amt_feehead6;
		}
		for($i=1;$i<=25;$i++)
		{
			$feehead[$i] 	= $this->dbcon->select('feehead','*',"ACT_CODE='$i' AND AccG=1");
	...
	for($i = 0; $i < $dataSize; $i++) 
	{
		...
		if($i==11)	$merchant_param1=$information[1];
	}
		...
		if(($on_orderid == $order_id) AND ($on_amt == $amount)  AND ($order_status == 'Success'))
		{   
	        if($pre_due > 0)
			{
		...
		$array =     array('school_logo'=>$school_logo,'school_name'=>$school_name,'school_add'=>$school_add,'school_phone'=>$school_phone,'school_aff'=>$school_aff,'school_web'=>$school_web,'school_email'=>$school_email,'orderr_id'=>$orderr_id,'pay_amt'=>$pay_amt,'order_status'=>$order_status,'adm_no'=>$admm,'rect_no'=>$ses,'merchant_param1'=>$merchant_param1);
