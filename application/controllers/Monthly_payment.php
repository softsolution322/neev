...
class Monthly_payment extends MY_controller
{
	...
	public function monthly_pay_details()
	{
		...
		//$get_daycoll = $this->db->query("select PERIOD from daycoll where RECT_NO='$rcpt_no'")->result();
		//$month = $get_daycoll[0]->PERIOD;
		$mon = explode("-", $ffm);
		...
		foreach ($mon as $key => $val) {
			...
			$check_mn = $val . '_FEE';
			$check_recpt = $this->dbcon->select('student', "$check_mn", "ADM_NO='$adm_no'");

			if ($check_recpt[0]->$check_mn != 'N/A' && $check_recpt[0]->$check_mn != '') {

				$upd_stu = array(

					$check_mn => $check_recpt[0]->$check_mn . ',' . $rcpt_no,
