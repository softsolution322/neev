...
table tr th,td{
	...
	@page { margin: 50px 5px 10px 20px; }
</style>
</head>
<body>
<div>
<!-- <p style='float:right; font-size:12px;'>Report Generation Date:<?php //echo date('d-M-Y'); ?></p><br /> -->
	<center><h1 style='font-size:31px;position:relative; top:-15px;'><?php echo 'SUNIL ENTERPRISES' ?></h1>
			<p style='font-size:22px; position:relative; top:-15px;'><?php echo $School_Address; ?>&nbsp;(<?php echo $School_Session ; ?>)</p>
			<!-- <p style='font-size:22px; position:relative; top:-25px;'>Session&nbsp;(<?php echo $School_Session; ?>)</p><br> -->
			<p style='font-size:22px;position:relative; top:-15px;'>Collection Report from <?php echo date('d-M-Y',strtotime($fromdate)); ?> To <?php echo date('d-M-Y',strtotime($todate)); ?></p>
...
<table border='1'>
	...
	<tbody>
		<?php
			...
			?>
			<!-- <td><?php //echo $data_type->Fee10;+$data_type->Fee11;+$data_type->Fee12;+$data_type->Fee14;+$data_type->Fee15;+$data_type->Fee21; ?></td> -->
							</tr>
						<?php
						$i++;
						$t = $t+$data_type->TOTAL;
...
<?php
	if($collection_type)
	{
		...
		foreach($collection_type as $collection_t)
		{
			...
			{
				$cash = $cash+$collection_t->TOTAL;
			}
			elseif($collection_t->Payment_Mode=='CARD SWAP')
			{
				$card = $card+$collection_t->TOTAL;
			}
			elseif($collection_t->Payment_Mode=='CHEQUE')
			{
				$cheque = $cheque+$collection_t->TOTAL;
			}
			elseif($collection_t->Payment_Mode=='UPI')
			{
				$upi = $upi+$collection_t->TOTAL;
			}
			elseif($collection_t->Payment_Mode=='ONLINE' || $collection_t->Payment_Mode=='Online'){
				$online = $online+$collection_t->TOTAL;
			}
			else{}
...
<p style='font-size:20px!important;'><u>Collection Statement Summary</u></p><br>
