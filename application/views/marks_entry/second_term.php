...
<ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="#">Second Term</a> <i class="fa fa-angle-right"></i></li>
</ol>
...
<script>
  function classes(val){
	  $.post("<?php echo base_url('Marks_entry/classess2'); ?>",{val:val},function(data){
  ...
  function secc(val){
	  ...
	  $.post("<?php echo base_url('Marks_entry/section2'); ?>",{val:val,Class_No:Class_No},function(data){
		  $("#exm_typ").html(data);
	  });
  }
  ...
  function exam_type(ExamCode){
	 ...
	 $.post("<?php echo base_url('marks_entry/subject2'); ?>",{ExamCode:ExamCode,Class_No:Class_No,ExamMode:ExamMode},function(data){
		 var fill = $.parseJSON(data);
		 $("#sub").html(fill[0]);
  ...
  function sorybyview(val){
	  ...
	  var ExamMode = $("#ExamMode").val();
	  var subcode    = $("#view_max_markss").val();
	 
	  
	  $.post("<?php echo base_url('Marks_entry/stu_list2'); ?>",{sortval:sortval,opt_code:opt_code,Class_No:Class_No,sec:sec,exm_code:exm_code,ExamMode:ExamMode,subcode:subcode},function(data){
		  $("#loading2").hide();
		  var fill = $.parseJSON(data);
  ...
  function marks(value){
	...
	if((MaxMarks >= vall) || (vall == 'ab') || (vall == '-')){
		...
		var mxm = splt[2];
		$.post("<?php echo base_url('Marks_entry/sv_nd_upd2'); ?>",{adm_no:adm_no,exm_typ:exm_typ,subcode:subcode,classs:classs,sec:sec,entr_val:entr_val,mxm:mxm},function(data){
			//alert(data);
		});
	}else{
		alert('Invalid No.');
